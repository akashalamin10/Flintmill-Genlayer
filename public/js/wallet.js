...
const ACCOUNT_KEY = "flintmill.account";
const WALLET_ID_KEY = "flintmill.walletId"; // rdns, "walletconnect", or "legacy-injected"
const DISCONNECTED_KEY = "flintmill.disconnected";
...
function bindProviderEvents(provider) {
  if (!provider?.on || provider.__flintmillBound) return;
  provider.__flintmillBound = true;
...
export function onAccountsChanged(handler) {
  window.addEventListener("flintmill:accountsChanged", (event) => handler(event.detail || ""));
}
